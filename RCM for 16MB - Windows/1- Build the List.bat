		...
		FOR /L %%c IN (0, 1, !count!) DO (
			...
			IF "!currentSha1!"=="!groupLine[1]!" (
				IF "!nameSelectionMode!" GTR "0" (
					...
					IF "!nameType!"=="0" (SET "name=!content[0]:~1!")
					IF "!nameType!"=="1" (SET "name=!content[1]:~1!")
					SET "fname[%%c]=!name!"
					SET /A "brackets=0"
					SET "content[0]="
					SET "content[1]="
				)
rem				SET "groupLine[2]=  !groupLine[2]!"
				SET "MSXType[%%c]=!groupLine[2]!"
				ECHO !fname[%%c]!
			)
		)
		SET /A "groupLineNumber=0"
...
FOR /L %%n IN (0,1,!maxfile!) DO (
	...
	SET "L=   !L!"
	SET "H=   !H!"
	ECHO 	db	!L:~-3!, !H:~-3!, !Type:~-3!, "  !sname:~0,38!">> !target!.asm
	SET maxlist=  0
)
...
FOR /L %%j IN (0, 1, !count!) DO (
	...
	FOR /R %1 %%k IN (*.txt) DO (
			...
			ECHO Applying patch on "!fname[%%~nxj]:~0,80!" >nul
			IPSPatcher "!sourceFile!" "!patchFile!" "!targetFile!" >nul
			IF EXIST "!targetFile!" (
				DEL "!sourceFile!"
